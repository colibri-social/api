/*
 * THIS FILE WAS GENERATED BY "@atproto/lex". DO NOT EDIT.
 */

import { l } from "@atproto/lex";
import * as SpaceDefs from "./defs.defs.js";

const $nsid = "com.atproto.space.getLatestCommit";

type $nsid = typeof $nsid;

export { $nsid };

export const $params = /*#__PURE__*/ l.params({"space":/*#__PURE__*/ l.string({"format":"space-ref"}),"repo":/*#__PURE__*/ l.string({"format":"did"})});

export type $Params = l.InferOutput<typeof $params>;

export const $output = /*#__PURE__*/ l.jsonPayload({"commit":/*#__PURE__*/ l.ref<SpaceDefs.SignedCommit>((() => SpaceDefs.signedCommit) as any)});

export type $Output<B = l.BinaryData> = l.InferPayload<typeof $output, B>;
export type $OutputBody<B = l.BinaryData> = l.InferPayloadBody<typeof $output, B>;

/** Get the current signed commit for an account's permissioned repo within a space. Served by a repo host. Throws RepoNotFound when the account holds no repo in the space, including the case of a member that has never written to it. Callable with either OAuth (for the authenticated user's own data) or a space credential (for syncing services). */
const main = /*#__PURE__*/ l.query($nsid, $params, $output, ["SpaceNotFound","RepoNotFound","RepoTakendown","RepoSuspended","RepoDeactivated"]);

export { main };

const $lxm = $nsid;

type $lxm = typeof $lxm;

export { $lxm };
