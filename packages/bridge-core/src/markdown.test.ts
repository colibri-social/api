...
import type { ColibriRichTextFacet } from "./markdown.js";
...
const kindOf = (feature: Feature): string => (feature.$type ?? "").split("#")[1] ?? "";

const summarize = (facets: Array<ColibriRichTextFacet>): Array<string> =>
	facets.map(
		(facet) =>
			`${facet.index.byteStart}-${facet.index.byteEnd}:${facet.features.map(kindOf).join("+")}`,
	);
...
const sliceFacet = (text: string, facet: ColibriRichTextFacet): string =>
	new TextDecoder().decode(
		new TextEncoder().encode(text).subarray(facet.index.byteStart, facet.index.byteEnd),
	);
...
describe("block prefixes", () => {
	it("puts a heading marker outside an inline mark starting at the same offset", () => {
		expect(facetsToSource("x", [buildFacet(0, 1, BOLD), buildFacet(0, 1, heading(1))]).source).toBe(
			"# **x**",
		);
	});

	it("ignores feature order within a merged facet", () => {
		expect(facetsToSource("x", [buildFacet(0, 1, BOLD, heading(1))]).source).toBe("# **x**");
		expect(facetsToSource("x", [buildFacet(0, 1, BOLD, list(false))]).source).toBe("- **x**");
	});

	it("emits one marker per kind when a range carries two of the same", () => {
		expect(
			facetsToSource("x", [buildFacet(0, 1, list(false)), buildFacet(0, 1, list(true))]).source,
		).toBe("- x");
		expect(
			facetsToSource("x", [buildFacet(0, 1, heading(1)), buildFacet(0, 1, heading(2))]).source,
		).toBe("# x");
	});
	...
	it("drops a prefix colliding with a fence rather than fencing the text twice", () => {
		expect(
			facetsToSource("code", [buildFacet(0, 4, list(false)), buildFacet(0, 4, CODEBLOCK)]).source,
		).toBe("```\ncode\n```");
	});
...
describe("round trip", () => {
	...
	it.each(cases)("is stable for %j", (source) => {
		...
		const second = parseMarkdown(facetsToSource(first.text, first.facets).source, []);
...
describe("nested lists", () => {
	const indentsOf = (source: string): Array<number> =>
		parseMarkdown(source, [])
			...
			.map((feature) =>
				"indent" in feature && feature.indent !== undefined ? Number(feature.indent) : 0,
			);
	...
	it("omits indent on a flat list", () => {
		const features = parseMarkdown("- a\n- b", [])
			.facets.flatMap((facet) => facet.features)
			.filter((feature) => kindOf(feature) === "list");
		for (const feature of features) expect(feature).not.toHaveProperty("indent");
	});
	...
	it("places markers after an astral character", () => {
		expect(roundTrip("- a \u{1f600}\n  - \u{1f600} b")).toBe("- a \u{1f600}\n  - \u{1f600} b");
		expect(roundTrip("\u{1f600} **bold** \u{1f389}")).toBe("\u{1f600} **bold** \u{1f389}");
	});
...
describe("parseMarkdown link policy", () => {
	...
	it("leaves other tokens alone when a link is rejected", () => {
		const { text, facets } = parseMarkdown("**b** [a](https://x.example)", [], rejectAll);
