...
export class PresenceTracker {
  ...
  private async settle(
    ...
    const derivedState: OnlineState =
      this.connections(did) > 0 ? ONLINE : OFFLINE;
    const row = {
      did,
      derivedState,
      ...
      viewingChannel:
        derivedState === OFFLINE ? null : (existing?.viewingChannel ?? null),
    ...
    this.deps.publish(
      ...
      memberships.map((membership) => membership.community),
      {
        ...
        presence: presenceOf(
          ...
          did,
          row,
          await loadActivity(this.deps.ctx, did),
        ),
      },
    );
  }
