...
const waitForConnection = async () => {
  for (let attempt = 0; attempt < 100 && connections.length === 0; attempt += 1)
    await settle();
...
describe("endpoint", () => {
  it("appends the v2 subscription path to a bare host", () => {
    expect(
      jetstreamEndpoint("wss://jetstream.us-west.bsky.network").toString(),
    ).toBe(
  ...
  it("leaves an explicit path alone", () => {
    const explicit =
      "wss://jetstream.internal/xrpc/network.bsky.jetstream.subscribeEvents";
    expect(jetstreamEndpoint(explicit).toString()).toBe(explicit);
  });

  it("recognises the legacy v1 endpoint", () => {
    expect(
      isLegacyJetstreamUrl("wss://jetstream1.us-east.bsky.network/subscribe"),
    ).toBe(true);
    expect(isLegacyJetstreamUrl("wss://jetstream.us-west.bsky.network")).toBe(
      false,
    );
  });
});
...
describe("subscription", () => {
  it("asks for identity, account, profile and listening commits only", async () => {
    ...
    expect(request.searchParams.getAll("kinds")).toEqual([
      "commit",
      "identity",
      "account",
    ]);
  ...
  it("carries a legacy microsecond cursor over unchanged", async () => {
    ...
    await startAgainstServer();
    expect((requests[0] as URL).searchParams.get("cursor")).toBe(
      "1787443064164880",
    );
  });
});
...
describe("profile commits", () => {
  ...
  it("replaces the cached colibri profile in place, leaving the bluesky one alone", async () => {
    ...
    socket.send(
      commitFrame(1, COLIBRI, "update", { $type: COLIBRI, displayName: "new" }),
    );
    await settle();
  ...
  it("replaces the cached bluesky profile without touching the colibri one", async () => {
    ...
    socket.send(
      commitFrame(1, BSKY, "update", { $type: BSKY, displayName: "new" }),
    );
    await settle();
  ...
  it("ignores a commit in a collection it did not ask for", async () => {
    ...
    socket.send(
      commitFrame(1, "app.bsky.feed.post", "create", { text: "hello" }),
    );
    await settle();
  ...
  it("still advances the cursor over a profile commit", async () => {
    ...
    socket.send(
      commitFrame(24990989093, COLIBRI, "update", { $type: COLIBRI }),
    );
    await settle();
    ...
    socket.send(
      commitFrame(1, ROCKSKY, "create", {
        ...
        track: {
          ...
          durationMs: 200597,
          albumCoverUrl:
            "https://i.scdn.co/image/ab67616d0000b273742e415e7f4bc3ee0c8ad600",
        },
